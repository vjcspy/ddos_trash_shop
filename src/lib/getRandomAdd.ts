...
export const getRandomAdd = async () => {
  ...
  const res = await fetch(`https://www.random-name-generator.com/vietnam?gender=&n=10&s=${randomIntFromInterval(5000, 100000)}`, {
    'headers': {
    ...
    medias.forEach(media => {
      ...
      if (Array.isArray(addText) && addText.length === 11) {
        ...
        if (textNodeElem.childNodes?.length === 3) {
          ...
          const add = (unescape(textNode.toString())).replace("\\n"," ").replace(/\s+/g," ");
          addDataArr.push(add);
        }
      }
      ...
      if(Array.isArray(addName) && addName.length === 1){
        if(Array.isArray(addName[0].childNodes) && addName[0].childNodes.length === 2){
          const nameNode = addName[0].childNodes[0];
           const name = (unescape(nameNode.toString())).replace("\\n"," ").replace(/\s+/g," ");
